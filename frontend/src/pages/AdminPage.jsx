...
const AdminPage = () => {
    ...
    try {
      ...
      try {
        const response = await axios.get(
          `${API_BASE_URL}/api/admin/qr-history`,
          {
    ...
    try {
      ...
      try {
        ...
        if (response.data) {
          ...
          //console.log(response.data.totalQRCodes)
          return; // Exit if successful
        }
      } catch (apiError) {
      ...
      try {
        ...
        setStats({
          ...
          recyclableItems:
            itemsResponse.data?.filter((item) => !item.isUsed)?.length || 0,
  ...
  const handleBulkGenerate = async () => {
    ...
    try {
      ...
      try {
        const response = await axios.post(
          `${API_BASE_URL}/api/admin/generate-bulk-qr`,
          {
            type: itemType,
            amount: bulkAmount,
          },
        ...
        if (response.data && response.data.success) {
          bulkSuccess = true;
          toast.success(
            `Successfully generated ${bulkAmount} QR codes in bulk`,
          );
        }

        if (response.data?.items) {
          setBulkQRCodes(response.data.items);
        }
      } catch (bulkError) {
      ...
      <div className="bg-emerald-600 text-white p-6 shadow-lg">
        <div className="container mx-auto">
          ...
          <p className="text-emerald-100 mt-1">
            Manage recyclable items and QR codes
          </p>
        </div>
      </div>
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-blue-700">
                  <p>
                    We're unable to connect to some admin API endpoints. The
                    dashboard is operating in compatibility mode.
                  </p>
                  <p className="mt-1">
                    If you're setting up a new backend, make sure to implement
                    these endpoints:
                  </p>
                  <ul className="list-disc list-inside mt-1 ml-2">
                    <li className="text-xs">
                      <code>/api/admin/stats</code> - For dashboard statistics
                    </li>
                    <li className="text-xs">
                      <code>/api/admin/qr-history</code> - For QR code history
                    </li>
                    <li className="text-xs">
                      <code>/api/items/generate-bulk-qr</code> - For bulk QR
                      generation
                    </li>
                  </ul>
                </div>
                ...
                <form onSubmit={handleGenerateQR} className="space-y-4">
                  ...
                  <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex items-center">
                      <div
                        className={`w-10 h-10 rounded-full ${getItemTypeColor(itemType)} flex items-center justify-center text-white`}
                      >
                        <FaRecycle />
                      </div>
                      <div className="ml-3">
                        ...
                        <p className="text-sm text-gray-500">
                          {getPointsForItemType(itemType)} points per item
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="w-full mt-6 grid grid-cols-3 gap-3">
                      <div className="bg-gray-50 p-3 rounded-lg text-center">
                        ...
                        <p className="font-mono font-medium text-gray-800 truncate text-sm">
                          {generatedQR.itemId}
                        </p>
                      </div>
                      <div className="bg-gray-50 p-3 rounded-lg text-center">
                        ...
                        <p
                          className={`font-medium capitalize ${getItemTypeColor(generatedQR.type)} text-white px-2 py-1 rounded text-xs inline-block`}
                        >
                          {generatedQR.type}
                        </p>
                      </div>
                  ...
                  <div className="h-full flex items-center justify-center">
                    <div className="text-center p-6 border-2 border-dashed border-gray-300 rounded-xl w-full">
                      ...
                      <p className="mt-1 text-sm text-gray-500">
                        Select an item type and click generate to create a new
                        QR code.
                      </p>
                    </div>
                  </div>
            ...
            >
              ...
              <div className="bg-emerald-50 p-4 rounded-lg mb-6 border border-emerald-100">
                <p className="text-sm text-emerald-800">
                  Generate multiple QR codes at once for efficient distribution.
                  The codes will be saved in the database and accessible through
                  the history tab.
                </p>
              </div>
                  ...
                  <form className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        max="50"
                        onChange={(e) =>
                          setBulkAmount(parseInt(e.target.value) || 1)
                        }
                ...
                <div>
                  <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-md">
                      ...
                      <dl className="divide-y divide-gray-200">
                        ...
                        <div className="py-2 grid grid-cols-2">
                          ...
                          <dd className="text-sm font-bold text-emerald-600">
                            {bulkAmount * getPointsForItemType(itemType)}
                          </dd>
                        </div>
                        <div className="py-2 grid grid-cols-2">
                          <dt className="text-sm font-medium text-gray-500">
                          ...
                          <dd className="text-sm text-gray-900">
                            ~{Math.ceil(bulkAmount / 10)} seconds
                          </dd>
                        </div>
                      </dl>
                    </div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {Array.from({ length: Math.min(10, bulkAmount) }).map(
                        (_, i) => (
                          <div
                            key={i}
                ...
                <div className="mt-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {bulkQRCodes.map((qr, index) => (
                      <div
                        key={qr.itemId}
                        className="p-4 bg-white border rounded-lg shadow flex flex-col items-center"
                      >
                        <QRGenerator data={qr.qrCode} />
                        ...
                        <span
                          className={`mt-1 px-2 py-0.5 rounded text-xs text-white ${getItemTypeColor(qr.type)}`}
                        >
                          {qr.type}
                        </span>
                      </div>
                      ...
                      {history.map((item, index) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`px-2 py-1 rounded text-xs font-medium text-white ${getItemTypeColor(item.type)}`}
                            >
                              {item.type}
                            </span>
                          </td>
